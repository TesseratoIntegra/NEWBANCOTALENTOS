...
type TabKey = 'tipos' | 'aguardando' | 'pendencias' | 'aprovados';
...
export default function DocumentosDashboardPage() {
  ...
  useEffect(() => {
    if (activeTab === 'aguardando' && awaitingCandidates.length === 0) {
      fetchAwaiting();
    }
  ...
  const tabs: { key: TabKey; label: string; count?: number }[] = [
    ...
    { key: 'pendencias', label: 'Pendências', count: pendingDocs.length || undefined },
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-zinc-400 text-sm mt-1">
          Gerencie tipos de documento, acompanhe pendências e perfis aguardando envio.
        </p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-center">
            <p className="text-zinc-400 text-sm">
              <span className="text-indigo-400 font-medium">{documentTypes.length}/10</span> tipos criados
            </p>
            ...
            <>
              ...
              {showNewForm && (
                <div className="bg-zinc-800 rounded-lg p-4 border border-indigo-500">
                  <h3 className="text-lg font-medium text-white mb-4">Novo Tipo de Documento</h3>
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-zinc-300 mb-1">Formatos Aceitos</label>
                        <input
                          type="text"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-zinc-300 mb-1">Descrição / Instruções</label>
                      <textarea
                    ...
                    <div className="flex flex-wrap gap-6">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor="is_required" className="text-sm text-zinc-300">Obrigatório</label>
                      </div>
                      <div className="flex items-center gap-2">
                        <label className="text-sm text-zinc-300">Tamanho máx.:</label>
                        <select
                          value={newType.max_file_size_mb || 5}
                          onChange={(e) => setNewType(prev => ({ ...prev, max_file_size_mb: parseInt(e.target.value) }))}
                          className="px-2 py-1 bg-zinc-700 border border-zinc-600 rounded text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        >
              ...
              {documentTypes.length === 0 ? (
                <div className="bg-zinc-800 rounded-lg p-12 text-center">
                  ...
                  <p className="text-zinc-400 mb-4">Nenhum tipo de documento criado ainda.</p>
                  <button
                    onClick={() => setShowNewForm(true)}
                ...
                <div className="space-y-3">
                  {documentTypes.map((type, index) => (
                    <div key={type.id} className="bg-zinc-800 rounded-lg p-4">
                      {editingId === type.id ? (
                        <div className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                            ...
                            <div>
                              <label className="block text-sm text-zinc-400 mb-1">Formatos</label>
                              <input
                                type="text"
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              rows={2}
                              className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                            />
                          </div>
                          <div className="flex flex-wrap gap-6">
                            <div className="flex items-center gap-2">
                              ...
                              />
                              <label className="text-sm text-zinc-300">Obrigatório</label>
                            </div>
                            <div className="flex items-center gap-2">
                              <label className="text-sm text-zinc-300">Tamanho máx.:</label>
                              <select
        ...
        <div className="space-y-4">
          ...
          {loadingAwaiting ? (
            ...
            <div className="bg-zinc-800 rounded-lg p-12 text-center">
              ...
              <p className="text-zinc-400">Todos os candidatos aprovados já completaram seus documentos obrigatórios.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {awaitingCandidates.map(candidate => {
                ...
                return (
                  <div key={candidate.candidate_id} className="bg-zinc-800 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <h3 className="text-white font-medium">{candidate.candidate_name}</h3>
                        <p className="text-zinc-500 text-sm">{candidate.candidate_email}</p>
                      </div>
                      <Link
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {candidate.not_sent_count > 0 && (
                        <span className="inline-flex items-center gap-1 px-2 py-1 bg-zinc-700 text-zinc-300 text-xs rounded-full">
                          <FileText className="h-3 w-3" />
                          {candidate.not_sent_count} não enviado{candidate.not_sent_count > 1 ? 's' : ''}
                        </span>
                      )}
                      {candidate.pending_count > 0 && (
                        <span className="inline-flex items-center gap-1 px-2 py-1 bg-yellow-900/50 text-yellow-300 text-xs rounded-full">
                      ...
                      {candidate.approved_count > 0 && (
                        <span className="inline-flex items-center gap-1 px-2 py-1 bg-green-900/50 text-green-300 text-xs rounded-full">
      ...
      {activeTab === 'pendencias' && (
        <div className="space-y-4">
          ...
          {loadingPending ? (
            ...
            <div className="bg-zinc-800 rounded-lg p-12 text-center">
              <CheckCircle className="h-12 w-12 text-green-500 mx-auto mb-4" />
              <p className="text-zinc-400">Nenhum documento pendente de revisão.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {Object.entries(pendingByCandidate).map(([candidateName, docs]) => (
                <div key={candidateName} className="bg-zinc-800 rounded-lg overflow-hidden">
                  <div className="px-4 py-3 bg-zinc-750 border-b border-zinc-700 flex items-center gap-2">
                    <Users className="h-4 w-4 text-indigo-400" />
                    <h3 className="text-white font-medium">{candidateName}</h3>
                    <span className="text-zinc-500 text-xs">({docs.length} documento{docs.length > 1 ? 's' : ''})</span>
                  </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-yellow-900/50 text-yellow-300 text-xs rounded-full">
                                <Clock className="h-3 w-3" />
                                Pendente
                              </span>
                            </div>
                        ...
                        {reviewingId === doc.id && (
                          <div className="mt-3 p-3 bg-zinc-700/50 rounded-lg">
                            ...
                            <textarea
                              ...
                              className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded-lg text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none text-sm"
                            />
                            <div className="flex gap-2 mt-2">
                              ...
                              <button
                                ...
                                className="px-4 py-1.5 bg-zinc-600 hover:bg-zinc-500 text-white rounded-lg text-sm transition-colors"
                              >
                                Cancelar
                              </button>
                            </div>
      ...
      {activeTab === 'aprovados' && (
        <div className="space-y-4">
          ...
          {loadingCompleted ? (
            ...
            <div className="bg-zinc-800 rounded-lg p-12 text-center">
              <FileText className="h-12 w-12 text-zinc-500 mx-auto mb-4" />
              <p className="text-zinc-400">Nenhum candidato com documentação completa ainda.</p>
            </div>
          ) : (
            <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        <h3 className="text-white font-medium">{candidate.candidate_name}</h3>
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-900/50 text-green-300 text-xs rounded-full font-medium">
                          <CheckCircle className="h-3 w-3" />
                          100% Aprovado
                        </span>
                      </div>
                      <p className="text-zinc-500 text-sm mt-1">{candidate.candidate_email}</p>
                      <div className="mt-2">
                        <div className="flex justify-between text-xs text-zinc-400 mb-1">
                          <span>{candidate.approved_count} de {candidate.total_required} obrigatórios aprovados</span>
                          <span>100%</span>
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      ...
                      <Link
                        ...
                        className="flex items-center gap-2 px-3 py-1.5 bg-green-700 hover:bg-green-600 text-white rounded-lg text-sm transition-colors"
                      >
