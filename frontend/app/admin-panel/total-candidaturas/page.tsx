...
function DetailsModal({ application, onClose, occupations }: DetailsModalProps) {
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
			<div className="bg-white border border-slate-200 rounded-xl p-8 w-full max-w-4xl relative shadow-xl max-h-[90vh] overflow-y-auto">
				<button
					onClick={onClose}
				...
				<div className="mb-6">
					<div className="flex items-center space-x-3 mb-4">
						...
						<div>
							<h2 className="text-2xl font-bold text-slate-900">{application.name}</h2>
						...
						<div className="space-y-3">
							...
							{application.number && (
								<div className="bg-slate-50 p-3 rounded-lg">
									...
									<p className="text-slate-900 mt-1">{application.number} {application.complement && `• ${application.complement}`}</p>
								</div>
							)}
						</div>
					</div>
				...
				<div className="mt-6 pt-6 border-t border-slate-300/50">
					...
					{application.type === 'spontaneous' ? (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div className="bg-slate-50 p-3 rounded-lg">
								<label className="text-xs font-medium text-slate-500 uppercase tracking-wide">Vaga</label>
								<p className="text-slate-900 mt-1">{application.job_title || 'Não informado'}</p>
							</div>
							<div className="bg-slate-50 p-3 rounded-lg">
								<label className="text-xs font-medium text-slate-500 uppercase tracking-wide">Empresa</label>
								<p className="text-slate-900 mt-1">{application.company_name || 'Não informado'}</p>
							</div>
						</div>
					)}
				</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						<div>
							...
							{application.resume ? (
								...
								<p className="text-slate-500">Currículo não disponível</p>
							)}
						</div>
						<div>
							<h3 className="text-lg font-semibold text-slate-900 mb-3 flex items-center">
							...
							<div className="bg-slate-50 p-3 rounded-lg">
								<p className="text-slate-900">{new Date(application.created_at).toLocaleString('pt-BR')}</p>
							</div>
						</div>
					</div>
				</div>
	...
	if (loading) {
		return (
			<div className="flex items-center justify-center min-h-64">
				<div className="text-slate-600 text-lg">Carregando candidaturas...</div>
			</div>
		);
	}
			...
			<div className="w-full mx-auto space-y-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
					<div className="bg-white rounded-lg border border-slate-200 shadow-sm p-6">
						<h3 className="text-slate-500 text-sm font-medium">Total</h3>
						<p className="text-slate-800 text-3xl font-bold mt-2">{applications.length}</p>
					</div>
					<div className="bg-white rounded-lg border border-slate-200 shadow-sm p-6">
						<h3 className="text-emerald-600 text-sm font-medium">Candidaturas para Vagas</h3>
						<p className="text-emerald-600 text-3xl font-bold mt-2">
							{applications.filter(app => app.type === 'normal').length}
						</p>
					</div>
					<div className="bg-white rounded-lg border border-slate-200 shadow-sm p-6">
						<h3 className="text-violet-600 text-sm font-medium">Candidaturas Espontâneas</h3>
						<p className="text-violet-600 text-3xl font-bold mt-2">
							{applications.filter(app => app.type === 'spontaneous').length}
						</p>
					</div>
					<div className="bg-white rounded-lg border border-slate-200 shadow-sm p-6">
						<h3 className="text-amber-600 text-sm font-medium">Filtrados</h3>
						<p className="text-amber-600 text-3xl font-bold mt-2">{filteredApplications.length}</p>
					</div>
				</div>
				...
				<div className="bg-white/50 backdrop-blur-sm border border-slate-200 rounded-lg p-6">
					<div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
						...
						<div className="flex items-center space-x-4">
							<label className="text-slate-600 font-medium">Filtrar por:</label>
							<select
								value={filterType}
								onChange={(e) => setFilterType(e.target.value as 'all' | 'normal' | 'spontaneous')}
								className="bg-slate-100 border border-slate-300 rounded-lg px-4 py-2 text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500"
							>
						...
						<div className="flex items-center">
							<button
								...
								className="flex items-center space-x-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white px-6 py-3 rounded-lg font-medium transition-colors cursor-pointer"
				...
				{/* Tabela */}
				<div className="w-full bg-white/30 backdrop-blur-sm border border-slate-200 rounded-lg shadow-lg overflow-hidden">
										...
										>
											<td className="px-6 py-4 whitespace-nowrap">
												<div className="flex items-center space-x-3">
													...
													<div>
														...
														{app.cpf && (
															<div className="text-xs text-slate-500">CPF: {app.cpf}</div>
														)}
													</div>
												</div>
											</td>
											...
											<td className="px-6 py-4">
												<div className="text-sm text-slate-900">
													{app.city}, {app.state}
													{app.neighborhood && (
														<div className="text-xs text-slate-500">{app.neighborhood}</div>
													)}
												</div>
											</td>
											<td className="px-6 py-4">
												<div className="text-sm">
													{app.type === 'normal' ? (
														<div>
															<div className="text-slate-900">{app.job_title || 'Vaga não informada'}</div>
															<div className="text-xs text-slate-500">{app.company_name || 'Empresa não informada'}</div>
															{app.status && (
																<div className="text-xs text-slate-500 mt-1">Status: {app.status}</div>
															)}
														</div>
													) : (
														<div className="space-y-1">
															{app.area_1 && (
																<div className="text-xs text-slate-600">• {getOccupationTitle(app.area_1)}</div>
															)}
															{app.area_2 && (
																<div className="text-xs text-slate-600">• {getOccupationTitle(app.area_2)}</div>
															)}
															{app.area_3 && (
																<div className="text-xs text-slate-600">• {getOccupationTitle(app.area_3)}</div>
															)}
														</div>
													)}
												</div>
